...
2. 면접 프로세스
   ...
   새 플로우:
3. Interview 생성 (status=SCHEDULED)
4. AI 질문 생성 → Questions 테이블 저장
5. Questions를 Transcripts에 AI 발화로 저장
6. 면접 시작 (status=LIVE)
7. 사용자 답변 → Transcripts에 User 발화로 저장
8. 실시간 감정 분석 → Transcripts.sentiment_score 업데이트
9. 면접 종료 (status=COMPLETED)
10. 최종 평가 리포트 생성 → Evaluation_Reports 테이블
11. Interview.overall_score 업데이트
12. 실시간 대화 기록
