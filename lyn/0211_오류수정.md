...
### **해결 내용**
- **코드 레벨 Fallback 구현:** `utils/exaone_llm.py`와 `tasks/question_generator.py`에서 새로운 경로(`/app/ai_models/`)에 파일이 없으면 자동으로 구형 경로(`/app/models/`)를 체크하도록 로직을 보강함.
- **안전성 확보:** 이제 호스트에서 파일 이동이 성공하든 실패하든, 워커는 어느 한 쪽에서라도 모델을 찾아내어 정상적으로 가동됨.
