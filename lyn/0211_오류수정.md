...
**보고서 작성 완료.**
코드는 사용자의 명시적 요청 전까지 수정되지 않았습니다.

---

## 5. LangSmith 도입 계획

네, 당연히 가능합니다! 현재 프로젝트가 이미 **LangChain**(0.2.0 이상) 기반으로 구성되어 있기 때문에, **LangSmith**를 추가하는 것은 매우 간단하며 디버깅과 성능 모니터링에 큰 도움이 될 것입니다.

### 🛠️ 코드 수정 및 도입 계획

LangSmith를 적용하기 위해 다음과 같은 단계로 작업을 진행하면 됩니다.

#### 1. 환경 변수 설정 (`.env` 수정)
LangSmith는 환경 변수만 설정되어 있으면 LangChain이 자동으로 트레이싱(Tracing)을 시작합니다. `.env` 파일에 다음 내용을 추가해야 합니다.

```bash
# LangSmith 설정
LANGCHAIN_TRACING_V2=true
LANGCHAIN_ENDPOINT="https://api.smith.langchain.com"
LANGCHAIN_API_KEY="your-langsmith-api-key" # 발급받은 API 키
LANGCHAIN_PROJECT="Big20-AI-Interview"    # 프로젝트명 (원하는 이름)
```

#### 2. Docker 설정 업데이트 (`docker-compose.yml` 수정)
컨테이너 환경에서 실행 중이므로, 새로운 환경 변수를 `ai-worker` 서비스 등에 전달해야 합니다.

*   `ai-worker-gpu`, `ai-worker-cpu`, 그리고 LLM을 호출할 가능성이 있는 `backend` 서비스의 `environment` 섹션에 위 변수들을 추가합니다.

#### 3. 의존성 추가 (`ai-worker/requirements.txt` 수정)
필수는 아니지만, 명시적으로 LangSmith SDK를 추가해두는 것이 좋습니다.

*   `langsmith` 패키지를 `requirements.txt`에 추가합니다.

#### 4. 코드 수준에서의 활용 (`ai-worker/utils/exaone_llm.py` 등)
현재 작성하신 `ExaoneLLM` 클래스는 이미 LangChain의 `LLM`을 상속받고 있으므로, 별도의 코드 수정 없이도 **모든 호출이 LangSmith에 자동으로 기록**됩니다.

만약 특정 체인(Chain)이나 호출에 태그를 달거나 프로젝트를 구분하고 싶다면, 호출 시 `config`를 추가할 수 있습니다:

```python
# 예시: 특정 호출에 태그 추가
llm.invoke("질문 내용", config={"tags": ["interview-stage-1"], "metadata": {"user_id": "123"}})
```
