...
### Q: 왜 Redis Pub/Sub을 중간에 사용했나요?

> **A**: "비동기 워커(Celery)와 실시간 통신 서버(FastAPI)가 분리되어 있기 때문입니다. 워커는 GPU 작업에 집중하고, 백엔드는 클라이언트 연결 관리에 집중하는 **관심사 분리(Separation of Concerns)**를 구현하기 위해 Pub/Sub 패턴을 도입했습니다."
