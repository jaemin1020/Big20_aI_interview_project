...
### 3.7. 🔌 WebRTC UDP 포트 및 Docker 네트워크 (NEW 2026-02-13)

#### 🔴 Issue: ICE Connection State 'checking' → 'failed' (무한 로딩)

* **발생일**: 2026-02-13
* **위치**: `media-server/main.py`, `docker-compose.yml`
* **증상**: SDP 교환은 성공했으나, ICE Connection State가 `checking`에서 멈추거나 `failed`로 떨어짐. 미디어 서버 로그에는 `⏳ 5초간 프레임 수신 없음 (타임아웃)`만 반복됨.
* **원인**:
  1. **UDP 포트 차단**: Docker 컨테이너는 기본적으로 외부에서 들어오는 UDP 패킷을 차단함. WebRTC는 미디어 전송에 UDP를 사용하므로 포트 개방이 필수.
  2. **랜덤 포트 할당**: `aiortc` 라이브러리는 OS에게 포트 할당을 맡겨 랜덤한 포트(Ephemeral Ports)를 사용함. Docker에서 이 모든 랜덤 포트를 미리 열어둘 수 없음.
* **시도 (실패)**:
  * `aioice.candidate.bind_socket`을 Monkey Patching하여 포트 범위를 강제하려 했으나, 해당 함수가 비공개 API이거나 존재하지 않아 `AttributeError` 발생, 서버 크래시 유발.
* **해결**:
  1. **Global Socket Monkey Patch**: Python 표준 `socket` 라이브러리를 **전역적으로 래핑(Wrap)**. `socket.bind()` 호출 시 UDP(`SOCK_DGRAM`)이고 포트가 0(랜덤)이면, 강제로 **50000~50050** 범위 내의 포트를 선택하도록 로직 수정.
  2. **Docker Port Mapping**: `docker-compose.yml`에 `50000-50050:50000-50050/udp` 포트 매핑을 명시적으로 추가하여 방화벽 해제.
* **결과**: 브라우저(외부)에서 Docker 내부로 UDP 패킷 진입 성공. `ICE Connected` 상태로 전환되며 실시간 영상 분석 로그 출력 시작.
