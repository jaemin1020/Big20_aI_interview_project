...
## 4. 📅 2026-02-19 ~ 02-23 작업 일지 — 시스템 통합 고도화 및 운영 안정화

> **작업 목표**: 실시간 면접 시스템의 안정성을 확보하고, 음성 데이터 기반의 '자신감 분석' 기능을 통합하며, 발생한 각종 네트워크 및 DB 연동 오류를 해결함.

---

### 4.1. 🌐 네트워크 및 인터페이스 (WebSocket & WebRTC)

#### 🔴 Issue #8: WebSocket 연결 실패 (ws://localhost:8080/ws/)
* **발생일**: 2026-02-19
* **위치**: `frontend` ↔ `media-server` 통신부
* **증상**: 프론트엔드에서 미디어 서버의 WebSocket 엔드포인트에 접속하지 못해 실시간 분석 데이터를 수신할 수 없음.
* **원인**: 
    1. Docker 컨테이너의 포트 매핑(`8080:8080`) 누락 또는 호스트 IP 접근 제한.
    2. `media-server`의 CORS 설정이 프론트엔드 도메인(`localhost:3000`)을 허용하지 않음.
* **해결**: `docker-compose.yml` 포트 설정 확인 및 FastAPI의 `CORSMiddleware` 재설정.

---

### 4.2. 👂 STT 및 오디오 분석 고도화

#### 🔴 Issue #9: STT 워커 큐 적체 (Worker Overload)
* **발생일**: 2026-02-19
* **위치**: `media-server/main.py` — `start_remote_stt()`
* **증상**: 면접자가 말을 빠르게 할 경우 오디오 청크가 큐에 무한정 쌓여 서버 응답 지연(60초 이상) 발생.
* **원인**: STT 서버의 처리 속도보다 클라이언트의 오딩 전송 속도가 빠름. (Backpressure 부재)
* **해결**: `pending_stt` 카운터와 `MAX_PENDING=2` 임계값을 도입. 현재 처리 중인 작업이 많을 경우 새 청크를 과감히 폐기(Drop)하여 실시간성 복구.

#### 🔴 Issue #10: 오디오 분석 기능 통합 충돌 (Merge Conflict)
* **발생일**: 2026-02-20
* **위치**: `media-server/main.py`
* **증상**: 메인 브랜치의 '큐 관리 로직'과 개인 브랜치의 '오디오 자신감 분석 로직'이 병합 과정에서 중복되거나 유실됨.
* **원인**: 수동 병합 과정에서 충돌 마커(`<<<< HEAD`)를 지우는 도중 로직 순서가 뒤섞임.
* **해결**: 
    1. `git reset --hard`로 머지 전 상태로 복구 후 재병합.
    2. '큐 적체 체크(문지기)' -> '자신감 분석(RMS/속도 계산)' -> 'STT 전송' 순서로 로직을 체계적으로 통합.

---

### 4.3. 🧠 백엔드 및 데이터베이스 (Backend & DB)

#### 🔴 Issue #11: DB 스키마 불일치 — `column users.is_withdrawn does not exist`
* **발생일**: 2026-02-20
* **위치**: `backend-core` 서비스 기동 시
* **증상**: `ProgrammingError`와 함께 백엔드 서버가 무한 재시작됨.
* **원인**: 백엔드 코드는 탈퇴 관리 기능(`is_withdrawn`, `withdrawn_at`)을 요구하나, 로컬 Docker DB 볼륨에는 해당 컬럼이 없는 구버전 스키마가 남아있음.
* **해결**: `docker compose down -v`를 통해 볼륨을 초기화하고 DB를 재생성하여 최신 스키마 적용.

---

### 4.4. 🚀 성능 최적화 (Advanced Optimization)

#### 🟡 Optimization #1: 비동기 스레드 풀 적용 (run_in_executor)
* **내용**: MediaPipe 전처리 로직이 메인 이벤트 루프를 블로킹하여 WebRTC 미디어 패킷 수신이 지연되는 현상 개선.
* **적용**: CPU 집약적 연산을 `loop.run_in_executor`를 통해 별도 스레드 풀에서 실행하도록 구조 변경.

#### 🟡 Optimization #2: 오디오 자신감 분석 로직 추가
* **내용**: 단순 텍스트 변환 외에, 오디오의 성량(RMS Volume)과 발화 밀도(Density)를 계산하여 면접자의 자신감을 수치화.
* **지표**: 성량 0.02 이상 시 유효 데이터로 간주, (성량점수*0.5 + 속도점수*0.5)로 계산.

---

### 4.5. 📋 2026-02-23 현재 상태 요약

| # | 작업 내용 | 최종 상태 | 비고 |
|---|----------|----------|------|
| 1 | 오디오 자신감 분석 알고리즘 구현 | ✅ 완료 | RMS/Density 기반 점수화 |
| 2 | STT 큐 관리 안정화 (MAX_PENDING) | ✅ 완료 | 서버 다운 방지 및 실시간성 확보 |
| 3 | 메인-개인 브랜치 코드 통합 | ✅ 완료 | `main.py` 충돌 해결 및 최적화 |
| 4 | DB 볼륨 초기화 및 스키마 동기화 | ✅ 완료 | 백엔드 정상 기동 확인 |

**향후 계획**: 최종 통합 테스트 진행 후, 면접 종료 리포트 UI에서 오디오 자신감 점수가 정상적으로 표시되는지 검증 예정.
