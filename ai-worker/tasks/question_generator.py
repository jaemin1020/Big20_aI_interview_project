    ...
    try:
        with Session(engine) as session:
            ...
            if next_stage.get("type") == "template":
                ...
                # [전략] AI 결과물 정제 (사족, 레이블, 특수 태그 제거)
                original_ai_output = final_content.strip()
                logger.info(f"🤖 [AI_RAW_OUTPUT] Stage: {next_stage['stage']}, Content: '{original_ai_output}'")

                # 1. 특수 레이블 및 마크다운 태그 제거
                final_content = re.sub(r'\[.*?\]', '', final_content) # [의사소통], [9번] 등 모든 대괄호 태그 제거
                ...
                meta_patterns = [
                    r'\(?이\s*질문은\s*.*?\)?', 
                    r'\(?질문\s*의도는\s*.*?\)?',
                    r'\(?.*?\s*검증하고자\s*하는\s*질문입니다\)?',
                    r'\(?.*?\s*확인하기\s*위한\s*질문입니다\)?',
                    r'지원자가\s*.*라고\s*답변했다면.*',
                    r'의도는\s*.*인\s*심층\s*질문입니다.*'
                ]
                for pattern in meta_patterns:
                    final_content = re.sub(pattern, '', final_content, flags=re.IGNORECASE | re.DOTALL).strip()

                # 3. 서두 레이블 및 불필요 문구 제거
                label_patterns = [
                    r'^\*{0,2}지원자의\s*답변\s*요약\s*및\s*꼬리질문:\*{0,2}\s*',
                    r'^\*{0,2}핵심\s*요약:\*{0,2}\s*',
                    r'^\*{0,2}꼬리질문:\*{0,2}\s*',
                    r'^\*{0,2}요약:\*{0,2}\s*',
                    r'^\*{0,2}질문:\*{0,2}\s*',
                    ...
                    r'^\d+\.\s*',
                    r'^-\s*',
                    r'^[:\s\.]+' # 처음에 붙는 콜론이나 점 제거
                ]
                for pattern in label_patterns:
                    final_content = re.sub(pattern, '', final_content, flags=re.IGNORECASE | re.MULTILINE).strip()

                # 4. [개선] 따옴표 추출 (물음표 유무와 상관없이 가장 긴 따옴표 안의 텍스트 추출 시도)
                # 이전: (r'["\'“]([^"\'”]*\?+)["\'”]') -> 물음표가 없으면 실패함
                quote_matches = re.findall(r'["\'“]([^"\'”]{10,})["\'”]', final_content) # 최소 10자 이상인 경우만
                if quote_matches:
                    # 가장 긴 것을 선택 (보통 질문이 가장 김)
                    final_content = max(quote_matches, key=len).strip()
                elif '?' in final_content:
                    # 따옴표가 없으면 마지막 물음표 기준으로 찌꺼기 제거
                    q_last_idx = final_content.rfind('?') + 1
                    if q_last_idx < len(final_content) and len(final_content) - q_last_idx < 60:
                        final_content = final_content[:q_last_idx]

                final_content = final_content.strip()
                final_content = final_content.replace("**", "").replace("__", "").strip()
                
                logger.info(f"✨ [CLEANED_STEP_1] Stage: {next_stage['stage']}, Content: '{final_content}'")

                # 5. [스테이지별 백지 방지 강화] 텍스트가 너무 짧거나 본문에 질문이 없는 경우 폴백
                sc_len = len(final_content.strip())
                if sc_len < 10: # 더 엄격하게 체크
                    logger.warning(f"⚠️ [Very Short Question Detected] Stage: {s_name}, Content: '{final_content}'")
                    if s_name == 'communication':
                        final_content = "팀 프로젝트 도중 팀원들과 의견 차이가 생겼을 때, 본인만의 방식으로 갈등을 조율하여 원만하게 해결했던 경험이 있다면 구체적으로 말씀해 주시겠습니까?"
                    elif s_name == 'growth':
                        final_content = "지원자님께서 지금까지 성장해 오면서 가장 중요하게 생각하는 삶의 가치나 철학은 무엇인지 말씀해 주시겠습니까?"
                    elif s_name == 'responsibility':
                        final_content = "지원자님의 평소 일에 임할 때 가장 중요하게 생각하는 본인만의 직업 윤리나 책임감은 무엇인가요?"
                    elif not final_content.strip():
                        final_content = "해당 주제와 관련하여 지원자님의 구체적인 생각이나 경험을 조금 더 자세히 들어보고 싶습니다. 말씀해 주시겠습니까?"
            ...
            # [최종 폴백] 정제 후 내용이 너무 짧거나 비어있으면 강제 할당
            sc = final_content.strip()
            if len(sc) < 15:
                logger.warning(f"⚠️ [Final Fallback Triggered] Stage: {next_stage['stage']}, Content: '{sc}'")
                s_name = next_stage.get('stage', '')
                if s_name == 'skill':
