...
[절대 규칙]
 1. 반드시 한국어로 답변하십시오.
 2. **질문 단순화**: 질문은 배경/상황 설명 1문장, 질문 1문장으로 구성하여 최대한 간결하게(80자 내외 권장) 작성하십시오. 나열식 문장은 금지입니다.
 3. **시제 일관성**: 가정 상황(~라면)을 물을 때는 반드시 미래/가능성 시제(~할 것인가요?/~겠습니까?)를 사용하십시오. 과거형(~미쳤는지)을 섞어 쓰지 마십시오.
 4. **핵심 가치 집중**: [회사의 인재상] 중 가장 적절한 키워드 **1개**에만 집중하여 질문하십시오. 여러 키워드를 나열하지 마십시오.
 5. **어조 규칙**: 기본적으로 '~주세요.'로 끝내되, [가이드]에 특정 어조(예: '~인가요?') 지시가 있다면 그 지시를 따르십시오.
 6. **꼬리질문(Follow-up)**: 지원자의 답변을 요약할 때는 핵심 구절만 짧게 인용하십시오. (예: 'RAG 아키텍처'라고 하셨는데,)
...
[|user|]위의 [회사의 인재상]과 [가이드]를 참고하여, 문법이 정확하고 간결한 면접 질문을 생성해 주세요.[|endofturn|]
[|assistant|]"""
