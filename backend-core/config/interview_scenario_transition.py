...
INTERVIEW_STAGES = [
    ...
    # 4. 직무 지식 꼬리질문 (AI 생성 - 답변 기반)
    {
        ...
        "type": "followup",
        "parent": "skill",
        "guide": "레이블(예: 요약, 질문 등) 없이 자연스럽게 지원자의 답변 내용을 한 문장의 서두로 요약하며 시작하고, 이어서 답변에서 언급된 기술, 방법론, 도구 중 하나를 선택하여 그 선택 이유나 구체적 활용 방식을 묻는 단일 꼬리질문을 생성하십시오.",
        "rubric": {
    ...
    # 6. 직무 경험 꼬리질문 (AI 생성 - 답변 기반)
    {
        ...
        "type": "followup",
        "parent": "experience",
        "guide": "레이블 없이 자연스럽게 이전 답변의 핵심 기술 키워드를 문장의 서두에 언급하며 시작하고, 이어서 그 개념의 정의나 실무 활용 방식을 묻는 한 문장의 질문을 작성하십시오. 반드시 어미는 '~인가요?' 혹은 '~무엇인가요?'로 끝내고 물음표를 포함하십시오.",
        "rubric": {
    ...
    # 8. 문제 해결 꼬리질문 (AI 생성 - 답변 기반)
    {
        ...
        "guide": "레이블 없이 자연스럽게 지원자의 답변을 '~라고 하셨는데,'와 같이 문장의 서두로 인용하며 시작하고, 이어서 해결 과정에서 고려했던 다른 대안이나 기술적 근거를 묻는 단일 질문을 던지십시오. 반드시 '~인가요?' 혹은 '~무엇인가요?' 어조를 사용하고 물음표를 포함하십시오.",
        "rubric": {
