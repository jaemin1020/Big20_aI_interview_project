...
INTERVIEW_STAGES = [
    ...
    # 4. 직무 지식 꼬리질문 (AI 생성 - 답변 기반)
    {
        ...
        "type": "followup",
        "parent": "skill",
        "guide": "지원자의 이전 답변을 언급하신 내용이라고 말씀해 주셨군요와 같이 한 문장으로 먼저 요약하십시오. 그 후 답변에서 언급된 구체적인 기술이나 수치, 방법론 중 하나를 콕 집어 그 이유나 상세 구현 방식을 묻는 심층 질문을 던지십시오. 모든 질문은 반드시 '주세요.'로 끝내고 물음표를 사용하지 마십시오.",
        "rubric": {
    ...
    # 8. 문제 해결 꼬리질문 (AI 생성 - 답변 기반)
    {
        ...
        "guide": "지원자의 이전 답변을 언급하신 내용이라고 하셨는데와 같이 한 줄 요약하며 인용하십시오. 그 후, 해결 과정에서 고려했던 다른 대안은 없었는지, 혹은 그 해결책이 최선이었다고 판단한 기술적 근거가 무엇인지 구체적으로 질문하십시오. 이 단계에서는 반드시 '인가요?' 혹은 '무엇인가요?' 어조를 사용하고 물음표를 포함하십시오.",
        "rubric": {
    ...
    # 12. 책임감 꼬리질문 (AI 생성 - 답변 기반)
    {
        ...
        "guide": "지원자의 이전 답변 중에서 책임감과 정직함이 가장 잘 드러난 문장을 짧게 요약하며 시작하십시오. 그 후, 만약 자신의 신념을 지키기 위해 개인적인 불이익을 감수해야 하는 딜레마 상황이 온다면 어떻게 본인의 가치를 지켜낼 것인지 1개의 단일 질문을 던지십시오. 모든 질문은 반드시 '...할 것인가요?'와 같이 '인가요?'로 끝맺음하고 물음표를 포함하십시오.",
        "order": 12
    },
    ...
    # 13. 성장의지 및 창의성 평가 (AI 생성 - 인재상 기반)
    {
        ...
        "type": "ai",
        "category": "narrative",
        "guide": "회사의 인재상인 '{company_ideal}'의 가치를 바탕으로, 지원자가 한 개인으로서 새로운 배움이나 변화를 마주할 때 어떤 마음가짐으로 스스로를 발전시키는지 본인만의 성장 철학을 묻는 질문을 생성하십시오. 직무 기술이나 IT 지식은 일절 언급하지 마시고, 순수한 성장 태도와 의지에만 집중해 주세요. 모든 질문은 반드시 '~주세요.'로 끝내고 물음표를 절대 사용하지 마십시오.",
        "order": 13
    },
    ...
    # 14. 성장의지 꼬리질문 (AI 생성 - 답변 기반)
    {
        ...
        "type": "followup",
        "parent": "growth",
        "guide": "지원자의 이전 답변에서 언급된 성장 태도를 언급하신 내용이라고 말씀하셨는데와 같이 인용하며 시작하십시오. 그 후, 스스로 설정한 목표가 뜻대로 되지 않거나 성장이 정체되었다고 느낄 때 본인만의 평정심을 유지하며 꾸준히 나아가는 방법이나 마음가짐에 대해 1개의 단일 질문을 던지십시오. 동료나 조직 상황은 언급하지 말고 오직 지원자 개인의 내면적 태도에만 집중하십시오. 모든 질문은 반드시 '주세요.'로 끝내고 물음표를 사용하지 마십시오.",
        "order": 14
    },
