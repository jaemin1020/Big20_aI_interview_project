...
INTERVIEW_STAGES = [
    ...
    # 11. 책임감 및 가치관 평가 (AI 생성 - 인재상 기반)
    {
        ...
        "type": "ai",
        "category": "narrative",
        "guide": "제공된 [지원자 자기소개서 질문1 답변]에서 지원자의 핵심 가치관이 드러난 문장을 반드시 그대로 혹은 가깝게 인용하며 (~라고 작성하셨는데,)로 질문을 시작하십시오. 이때 백엔드나 프로젝트 같은 직무 세부 사항은 절대 언급하지 마십시오. 오직 인용된 가치관과 회사의 인재상인 '{company_ideal}'을 연결하여, 비전공자로서 새로운 직군(IT)에 임할 때 어떤 기준과 책임감을 가지고 업무를 수행할 것인지 묻는 1개의 단일 질문을 생성하십시오. 모든 질문은 반드시 '~주세요.'로 끝내고 물음표를 절대 사용하지 마십시오.",
        "order": 11
    },
