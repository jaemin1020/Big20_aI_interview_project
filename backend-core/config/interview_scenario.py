...
INTERVIEW_STAGES = [
    ...
    # 8. 문제 해결 꼬리질문 (AI 생성 - 답변 기반)
    {
        ...
        "guide": "지원자의 이전 답변을 언급하신 내용이라고 하셨는데와 같이 한 줄 요약하며 인용하십시오. 그 후, 해당 해결책을 선택한 기술적 근거가 무엇인지, 혹은 예상치 못한 변수나 한계점은 없었는지 질문하십시오. 이 단계에서는 반드시 '인가요?' 혹은 '무엇인가요?' 어조를 사용하고 물음표를 포함하십시오.",
        "rubric": {
    ...
    # 12. 책임감 꼬리질문 (AI 생성 - 답변 기반)
    {
        ...
        "guide": "지원자의 이전 답변 중에서 책임감과 정직함이 가장 잘 드러난 문장을 짧게 요약하며 시작하십시오. 그 후, 만약 자신의 신념을 지키기 위해 개인적인 불이익을 감수해야 하는 딜레마 상황이 온다면 어떻게 본인의 가치를 지켜낼 것인지 딱 1개의 예리한 단일 질문을 던지십시오. 생성된 질문은 반드시 '...할 것인가요?'와 같이 '인가요?'로 끝맺음하고 단 하나의 물음표만 포함하십시오.",
        "order": 12
    },
