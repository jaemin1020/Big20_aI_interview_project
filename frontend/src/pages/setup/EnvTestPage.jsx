  ...
  useEffect(() => {
    ...
    const startAudioAnalysis = async () => {
      ...
      try {
        ...
        // audioContext.destination에 연결하지 않음 (피드백 방지)
        // javascriptNode.connect(audioContext.destination);  // 주석 처리!
        ...
        // 2. Whisper STT (백엔드 API 호출)
        // 주의: stream은 원본 마이크 스트림 사용 (audioContext 거치지 않음)
        const mediaRecorder = new MediaRecorder(stream, {
          mimeType: 'audio/webm;codecs=opus'  // opus 코덱 명시
        });
        const audioChunks = [];

        console.log('MediaRecorder 생성 완료:', mediaRecorder.mimeType);
        ...
        mediaRecorder.addEventListener('stop', async () => {
          ...
          console.log('녹음 완료! 총 크기:', audioBlob.size, 'bytes');
          ...
          try {
            ...
            if (response.ok) {
              console.log('STT API Raw Response:', response);
              const result = await response.json();
              console.log('STT API JSON Result:', result);
              const text = result.text || '';
              if (text.trim()) {
                setTranscript(prev => (prev ? prev + ' ' : '') + text);
                setIsRecognitionOk(true);
                console.log('Whisper STT:', text);
              }
            } else {
          ...
          // 다시 녹음 시작
          if (mediaRecorder.state === 'inactive' && step === 'audio') {
